...
3. 同一个实验下，同一个代下，不同的worker用的是不同的seed。

noise:

实验记录:
1. 记录每次实验的平均reward 完成
2. 记录每次实验的平均novelty 完成
3. 记录mean_bc的archive
...
def run_master(master_redis_cfg, log_dir, exp):
    ...
    mean_bc_list = []
    filepath = os.path.join('/home/ubuntu/workspace/research/deep-neuroevolution/fixed_dict.pickle_1')
    fixed_dict_fixed = pickle.load(open(filepath,'rb'))
    theta_dict_fixed = fixed_dict_fixed['theta_dict']
    ...
    fixed_dict['theta_dict'] = theta_dict
    fixed_dict['optimizer_dict'] = optimizer_dict
    fixed_dict['mean_bc_list'] = mean_bc_list
    fixed_dict['ref_batch'] = ref_batch
    archive = master.get_archive()
    fixed_dict['archive'] = archive
    ...
    while True:
        ...
        debug_dict['signnovelty_mean'] = signreturns_n2_mean
        # reward
        returns_n2_mean = np.mean(returns_n2, axis=0)
        debug_dict['returns_n2_mean'] = returns_n2_mean
