...
def run_master(master_redis_cfg, log_dir, exp):
    ...
    optimizer_dict = deepcopy(fixed_dict_fixed['optimizer_dict'])
    # fixed_dict['mean_bc_list'] = mean_bc_list
    mean_bc_list = deepcopy(fixed_dict_fixed['mean_bc_list'])
    archive = deepcopy(fixed_dict_fixed['archive'])
    ...
    while True:
        ...
        debug_dict['signnovelty_mean'] = deepcopy(signreturns_n2_mean)
        # reward
        returns_n2_mean = np.mean(returns_n2, axis=0)
        debug_dict['returns_n2_mean'] = deepcopy(returns_n2_mean)
        ...
        if exp['novelty_search']['selection_method'] == "novelty_prob":
            ...
            debug_dict['archive'] = deepcopy(archive)
            for p in range(pop_size):
